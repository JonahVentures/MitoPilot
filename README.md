...
assmebly with GetOrganelle or MitoFinder. We have provided some
[documentation](https://smithsonian.github.io/MitoPilot/articles/custom_dbs.html)
for building custom reference databases.

However, the custom logic in the annotation curation and validation
scripts needs to be tweaked for optimal performance with other taxonomic
groups. Because all of the curation rulesets are contained in the
underlying Docker image (currently hosted at
...
We have implemented curation/validation rulesets for the following
groups of organisms:
...
setting up an example project (`new_test_project()`), and [tutorial

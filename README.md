...
The reference databases and annotation parameters can be easily updated.
However, the custom logic in the annotation curation and validation
scripts would likely need to be updated for optimal performance with
other taxonomic groups.

Because all of the dependencies and reference data is contained on the
underlying Docker Image (currently hosted at `macguigand/MitoPilot`),
...
included in this repository for reference and a custom local Docker
...
To use your own mitogenome assemblies, first create a mapping file with
two additional columns: `Assembly` and `Topology`. The `Assembly` column
should contain the names of your mitogenome FASTA files. Ideally, each
FASTA file should contain a single contig or scaffold representing the
complete mitogenome. The names of the FASTA files and the sequence
headers in the files do not matter. The `Topology` column should
indicate whether your assembly is “linear” or “circular”.

process assemble_MF {
    ...
    input:
    tuple val(id), val(opts_id), path(reads), val(opts), path(dbs), val(genetic_code)
    ...
    tuple val("${id}"),
        ...
        path("${id}/assemble/${opts_id}/${id}_summary.txt"),                  // Mitofinder summary
        val("${opts_id}"),                                                    // options id
        path("${id}/assemble/${opts_id}/mitofinder.log.txt"),                     // MitoFinder log
...
*/
    ...
    mkdir -p !{outDir}
    ### LOGS ####
    cp !{workingDir}/*_MitoFinder.log !{outDir}/mitofinder.log.txt

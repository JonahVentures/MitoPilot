...
N_REC=$( esearch -db nucleotide -query "${QUERY}" | efilter -location "mitochondrion" -molecule "genomic" -source "refseq" | grep "Count" | cut -f2 -d">" | cut -f1 -d"<" )

echo "Found ${N_REC} records matching this query, downloading now"
echo "This may take a few minutes..."

esearch -db nucleotide -query "${QUERY}" | efilter -location "mitochondrion" -molecule "genomic" -source "refseq" | efetch -format gb > genbank.gb
